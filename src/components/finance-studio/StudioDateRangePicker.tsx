...
export function StudioDateRangePicker({ startDate, endDate, onChange, placeholder = "Select date range", className, align = 'left' }: StudioDateRangePickerProps) {
    ...
    return (
        <div className={`relative ${className ?? ''}`} ref={containerRef}>
            ...
            >
                <span className={`text-[12.5px] font-[500] font-mono ${hasValue ? 'text-gray-900' : 'text-gray-400'}`}>
                    {displayText()}
                </span>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className={`absolute top-full mt-1.5 z-[9999] bg-white rounded-[10px] p-4 w-[272px] select-none overflow-hidden
                    ...
                    >
                                            ...
                                            return (
                                                <button type="button" key={idx} onClick={() => handleDayClick(day)}
                                                    className={`h-7 w-7 rounded-[5px] flex items-center justify-center text-[11.5px] font-[500] transition-all mx-auto
                                                        ...
                                                        ${rangeStart || rangeEnd ? '!bg-[#6366F1] !text-white shadow-sm' : ''}
                                                        ${inRange && !rangeStart && !rangeEnd ? 'bg-indigo-50 text-[#6366F1]' : ''}
                                                        ${isTodayDate && !rangeStart && !rangeEnd && !inRange ? '!text-[#6366F1] font-[700] bg-indigo-50' : ''}
                                                    `}>
                                                    {format(day, "d")}
                                                </button>
                                            );
                        ...
                        <div className="mt-2 pt-3 flex justify-between items-center" style={{ borderTop: HAIRLINE }}>
                            <button type="button"
                                onClick={() => { setTempStartDate(null); setTempEndDate(null); onChange(null, null); }}
                                className="text-[10.5px] font-[600] text-gray-400 hover:text-gray-600 uppercase tracking-[0.07em]">
                                Clear
                            </button>
                            <button type="button"
                                onClick={() => {
