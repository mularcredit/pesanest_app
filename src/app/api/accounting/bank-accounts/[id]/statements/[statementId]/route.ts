...
export async function DELETE(req: Request, props: { params: Promise<{ id: string; statementId: string }> }) {
    ...
    if (!statement || (statement.bankAccountId !== params.id && statement.paybillAccountId !== params.id && statement.walletId !== params.id && statement.paystackAccountId !== params.id)) {
